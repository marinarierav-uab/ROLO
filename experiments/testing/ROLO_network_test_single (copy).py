...
class ROLO_TF:
    ...
    def dropout_features(self, feature, prob):
        ...
        drop_index = random.sample(range(4096), num_drop)
    ...
    def ROLO(self, argvs):
            ...
            if self.rolo_utils.flag_train is True:
                ...
                x_path = os.path.join('cvc-data', sequence_name, 'yolo_out/')
                y_path = os.path.join('cvc-data', sequence_name, 'groundtruth_rect.txt')
                self.output_path = os.path.join('cvc-data', sequence_name, 'rolo_out_test/')
                utils.createFolder(self.output_path)

                self.rolo_weights_file = 'checkpoints/model_demo.ckpt'
